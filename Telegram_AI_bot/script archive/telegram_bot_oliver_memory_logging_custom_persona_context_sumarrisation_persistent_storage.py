...
from telegram.ext import (
    ...
    ConversationHandler, CallbackQueryHandler, PicklePersistence # Import PicklePersistence
)
import sys
import os
from dotenv import load_dotenv
import json # New import
import asyncio # New import, for completeness if async operations are used with saving, though not strictly needed for this specific `save_conversation_history`
...
PERSISTENCE_DIR = "persistence" # New constant for persistence directory

os.makedirs(LOGS_DIR, exist_ok=True)
os.makedirs(USER_LOGS_DIR, exist_ok=True)
os.makedirs(PERSISTENCE_DIR, exist_ok=True) # Ensure persistence directory exists
...
CONVERSATION_HISTORY_FILE = os.path.join(PERSISTENCE_DIR, "conversation_history.json")
    ...
    if os.path.exists(CONVERSATION_HISTORY_FILE):
        try:
            with open(CONVERSATION_HISTORY_FILE, 'r', encoding='utf-8') as f:
                ...
                # Ensure chat_ids are integers if they were serialized as strings
                return {int(k): v for k, v in loaded_data.items()}
        except json.JSONDecodeError as e:
            logger.error(f"Error decoding conversation history JSON: {e}")
            return {}
...
async def save_history_job(context: ContextTypes.DEFAULT_TYPE):
    """Job to periodically save conversation history."""
    save_conversation_history()
    logger.debug("Periodic save of conversation history triggered.")

SAVE_INTERVAL_SECONDS = 300 # Save every 5 minutes

# --- Conversational Memory Storage ---
conversation_history = load_conversation_history() # Load history on startup
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Initialize or reset token count for the new conversation (from previous enhancement)
    context.user_data['current_conversation_tokens'] = 0
    user_logger.info(f"Token count initialized to 0 for chat_id: {chat_id}")
...
async def receive_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Initialize token count for the new conversation (from previous enhancement)
    context.user_data['current_conversation_tokens'] = 0
    user_logger.info(f"Token count initialized to 0 for chat_id: {chat_id}")
...
async def persona_chosen_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if callback_data == "ignore_me":
        ...
        if chosen_system_prompt:
            ...
            # Initialize or reset token count for the new persona context (from previous enhancement)
            context.user_data['current_conversation_tokens'] = 0
...
async def save_custom_persona(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Initialize or reset token count for the new custom persona context (from previous enhancement)
    context.user_data['current_conversation_tokens'] = 0
    user_logger.info(f"Token count initialized to 0 after custom persona activation for chat_id: {chat_id}")
...
async def clear_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in conversation_history:
        ...
        # Reset token count as history is cleared (from previous enhancement)
        context.user_data['current_conversation_tokens'] = 0
...
# Add this constant for token limit (from previous enhancement)
MAX_CONVERSATION_TOKENS = 4000 # Example limit, adjust based on your LLM's context window and desired usage
...
async def chat_with_lm_studio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id not in conversation_history:
        ...
        # Ensure token count is initialized for new conversations (from previous enhancement)
        context.user_data['current_conversation_tokens'] = 0 
        user_logger.info(f"New conversation started for chat_id: {chat_id}. Token count initialized.")

    current_system_prompt = context.user_data.get('llm_system_prompt', DEFAULT_LM_STUDIO_SYSTEM_PROMPT)

    # --- Token Limit Check (from previous enhancement) ---
    current_tokens = context.user_data.get('current_conversation_tokens', 0)
    if current_tokens >= MAX_CONVERSATION_TOKENS:
        user_logger.info(f"Token limit reached for chat_id: {chat_id} ({current_tokens}/{MAX_CONVERSATION_TOKENS}). Suggesting clear.")
        await update.message.reply_text(
            f"🚫 Our conversation has reached its maximum token limit ({MAX_CONVERSATION_TOKENS} tokens).\n\n"
            "Please use the `/clear` command to start a fresh conversation, or `/persona` to switch roles, which also clears the history."
        )
        return # Stop processing the message if limit is reached
    ...
    context.user_data['last_user_message_context'] = {
        ...
        "history_at_time": list(conversation_history[chat_id])
    }
    ...
    try:
        ...
        lm_studio_response = ""
        prompt_tokens_used = 0 # From previous enhancement
        completion_tokens_used = 0 # From previous enhancement
        ...
        for chunk in completion:
            ...
            # Accumulate token usage from the last chunk if available (for non-streaming, it's in the main object)
            if chunk.usage: 
                prompt_tokens_used += chunk.usage.prompt_tokens or 0
                completion_tokens_used += chunk.usage.completion_tokens or 0
        ...
        # Update the total token count for the user in context.user_data (from previous enhancement)
        if 'current_conversation_tokens' not in context.user_data:
            context.user_data['current_conversation_tokens'] = 0
        total_tokens_this_turn = prompt_tokens_used + completion_tokens_used
        context.user_data['current_conversation_tokens'] += total_tokens_this_turn
        
        user_logger.info(f"LM Studio response: {lm_studio_response}")
        user_logger.info(f"Tokens used in this turn (prompt: {prompt_tokens_used}, completion: {completion_tokens_used}). Total for conversation: {context.user_data['current_conversation_tokens']}")
...
async def regenerate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if lm_studio_response:
            ...
            await response.message.edit_text("Sorry, I didn't get a clear regenerated response from the AI.")
        ...
        user_logger.info(f"Tokens used in regeneration turn (prompt: {prompt_tokens_used}, completion: {completion_tokens_used}). Total for conversation: {context.user_data['current_conversation_tokens']}")
...
async def shutdown_handler(application):
    """Handler for graceful shutdown to save data."""
    logger.info("Bot shutting down. Saving all data...")
    save_conversation_history() # Save custom history
    # PicklePersistence handles its own saving on shutdown if configured correctly
    logger.info("Data saved. Goodbye!")

def main():
    # Create a persistence object for user_data, chat_data, bot_data
    ...
    # Schedule the periodic saving of conversation history
    application.job_queue.run_repeating(save_history_job, interval=SAVE_INTERVAL_SECONDS, first=5) # Start after 5 seconds
